...
const getModelRegiaoMidia = () => ({
    _id: uuidv4(),
    texto: '',
    midias: [],
})

class FormRegiaoMidia extends Component {
...
FormRegiaoMidia.defaultProps = {
    placeholder: 'RegiaoMidia'
}

class RegiaoMidia extends Component {
    ...
    state = {
        ...
        open: false,
        toDelete: '',
        itens: this.props.defaultValue.length > 0 ? this.props.defaultValue : [getModelRegiaoMidia()]
    }
                ...
                <List style={{ marginRight: "10px" }}
                    ...
                    renderItem={(item, idx) => (
                        <Item key={item._id} actions={[
                            ...
                            <FormRegiaoMidia placeholder={this.props.placeholder} name={name} sinalNovo={sinalNovo} onEnter={this.onAdd} loading={loading} item={item} idx={idx} onChange={this.onChange} />
                        </Item>)}
                />
                <Modal
    ...
    onDelete = () => {
        ...
        if (itens.length == 1) {
            this.setState({ itens: [getModelRegiaoMidia()] })
        } else {
            this.setState({
                itens: [
...
RegiaoMidia.defaultProps = {
    defaultValue: [getModelRegiaoMidia()]
}

export default RegiaoMidia;
