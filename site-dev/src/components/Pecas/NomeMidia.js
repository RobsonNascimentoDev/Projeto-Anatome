...
const getModelNomeMidia = () => ({
    _id: uuidv4(),
    texto: '',
    midias: [],
})

class FormNomeMidia extends Component {
...
FormNomeMidia.defaultProps = {
    placeholder: 'NomeMidia'
}

class NomeMidia extends Component {
    ...
    state = {
        ...
        open: false,
        toDelete: '',
        itens: this.props.defaultValue.length > 0 ? this.props.defaultValue : [getModelNomeMidia()]
    }
                ...
                <List style={{ marginRight: "10px" }}
                    ...
                    renderItem={(item, idx) => (
                        <Item key={item._id} actions={[
                            ...
                            <FormNomeMidia placeholder={this.props.placeholder} name={name} sinalNovo={sinalNovo} onEnter={this.onAdd} loading={loading} item={item} idx={idx} onChange={this.onChange} />
                        </Item>)}
                />
                <Modal
    ...
    onDelete = () => {
        ...
        if (itens.length == 1) {
            this.setState({ itens: [getModelNomeMidia()] })
        } else {
            this.setState({
                itens: [
...
NomeMidia.defaultProps = {
    defaultValue: [getModelNomeMidia()]
}

export default NomeMidia;
